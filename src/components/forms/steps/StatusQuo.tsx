...
function NumBadge({ n }: { n: number }) {
  return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full text-[13px] font-semibold bg-orange-100 text-gray-800">
      {n}
    </span>
  )
}
...
function SubTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-base font-semibold flex items-center gap-2 text-gray-800 font-serif">
      {children}
    </h3>
  )
}
...
export default function StatusQuo() {
  return (
    <div className="space-y-8">
      <h1 className="text-2xl md:text-[28px] font-semibold tracking-[.02em] text-gray-800 font-serif">
        Status Quo Analyse
      </h1>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <div className="uppercase text-[11px] font-semibold tracking-[.12em] flex items-center gap-2 text-gray-800">
              <MapPinIcon /> STANDORT
            </div>
            <div className="text-sm text-gray-600">Flurstraße 8, 90451 Nürnberg-Eibach</div>
            <div className="rounded-[3px] bg-white/95 shadow-[0_1px_3px_rgba(0,0,0,0.22)] w-full h-48 flex items-center justify-center text-sm text-gray-500">
              Kartenplatzhalter
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {['87%', '82%', '79%'].map((score, idx) => (
            <div key={idx} className="space-y-2">
              <div className="text-[12px] inline-flex items-center px-2 py-1 rounded-full bg-blue-50 text-gray-800">
                Matchscore: {score}
              </div>
              <div className="rounded-[3px] overflow-hidden bg-white/95 shadow-[0_1px_3px_rgba(0,0,0,0.22)] aspect-[4/3] flex items-center justify-center text-gray-500">
                Bildplatzhalter
              </div>
            </div>
          ))}
