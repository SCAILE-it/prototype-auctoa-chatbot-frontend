...
export default function Tabs({ active, onChange }: TabsProps) {
  return (
    <nav role="tablist" aria-label="Schritte" className="p-6 md:p-8 pb-2">
      <div className="flex w-full items-center gap-x-3 gap-y-2 text-sm bg-[rgba(238,238,238,0.88)] rounded-[3px] px-3 py-2 flex-wrap xl:flex-nowrap min-w-0">
        {steps.map((s, i) => (
          <div key={s.n} className="flex items-center gap-3 min-w-0">
            ...
            >
              <span className={[
                ...
                s.n === active ? 'border-black text-black bg-white' : 'border-black/20 text-[#999999] bg-white'
              ].join(' ')}>{s.n}</span>
